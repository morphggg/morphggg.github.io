...
document.addEventListener('DOMContentLoaded', () => {
    ...
    list.addEventListener('click', async (e) => {
        ...
        if (action === 'remove') {
            ...
            const tech = card.querySelector('[data-group-tech]');
            const expanded = btn.getAttribute('aria-expanded') === 'true';
            btn.setAttribute('aria-expanded', String(!expanded));
            btn.setAttribute('aria-label', !expanded ? 'Свернуть данные' : 'Развернуть данные');
            tech.hidden = expanded;
        } else if (action === 'edit') {
            openModal('edit', card);
        }
    });
...
function fillCard(card, data) {
    ...
    // Видимые поля в .group-details
    const pwEl = card.querySelector('.group-details [data-field="password"]');
    if (pwEl) pwEl.textContent = data.password ? '••••••••' : '—';
    const loginEl = card.querySelector('.group-details [data-field="login"]');
    if (loginEl) loginEl.textContent = data.login || '—';
    const linkEl = card.querySelector('.group-details [data-field="link"]');
    if (linkEl) { linkEl.textContent = data.link || '—'; linkEl.href = data.link || '#'; }
    // Заголовок
